...
- Configure options in setup:
  ...
  - Bonus answers On/Off (radio)
- Bonus answers randomly assign one non-top answer in each round, play `bonus.mp3` on reveal, and highlight it yellow.
- In the final round, the Next Round button changes to **End Game**, then shows the winner (with emoji) and the other team score.
- End game view now hides the active board and shows a large winner screen.
- Team strike ❌ indicators are enlarged for better classroom visibility.
